...
class Command(BaseCommand):
    help = 'Import dữ liệu bus stops vào model Point'
    ...
    def handle(self, *args, **options):
        ...
        created = 0
        for lat, lon, html in data:
            soup = BeautifulSoup(html, 'html.parser')
            title = soup.b.get_text(strip=True)

            parts = [p.strip() for p in re.split(r'<br\s*/?>', html) if p.strip()]
            address = BeautifulSoup(parts[1], 'html.parser').get_text(strip=True)
            routes_txt = BeautifulSoup(parts[2], 'html.parser').get_text(strip=True)
            routes = routes_txt.replace('Các tuyến đi qua:', '').strip()
        ...
        self.stdout.write(self.style.SUCCESS(
            f"✅ Đã chèn thành công {created} bản ghi vào model Point."
        ))
